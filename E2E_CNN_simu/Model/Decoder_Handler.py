...
class Decoder_Handler(Basement_Handler):
    ...
    def __init__(self, dataset_name, model_config, sess, is_training=True, is_testing_meas=False):  #zzh
        ...
        self.gen_train = Data_Generator_File(dataset_name,self.train_index,self.sense_mask,self.batch_size,self.nF,
                                             is_training=True,is_valid=False,is_testing=False)
        self.gen_valid = Data_Generator_File(dataset_name,self.valid_index,self.sense_mask,self.batch_size,self.nF,
                                             is_training=False,is_valid=True,is_testing=False)
        self.gen_test = Data_Generator_File(dataset_name,self.test_index,self.sense_mask,self.batch_size,self.nF,
                                            is_training=False,is_valid=False,is_testing=True, is_testing_meas=is_testing_meas)
    ...
    def data_assignment(self,dataset_name, is_testing=False):
        # Division for train, test and validation
        model_config = self.model_config
        train_index, valid_index, test_index, self.sense_mask = Data_Division(dataset_name,self.nF, is_testing=is_testing)
        ...
        print('\ntraining_sample_number is '+ str(disp_train))
    ...
    def test(self):
        ...
        for tested_batch in range(self.test_size):
            ...
            print('One batch reconstructed in %s\n' % (time.time()-start2))
            ...
            matcontent_v[u'pred'],matcontent_v[u'meas'], matcontent_v[u'truth']= \
                np.squeeze(test_output['pred_orig']), np.squeeze(measure_test), np.squeeze(ground_test)
            hdf5storage.write(matcontent_v,'.',self.log_dir+'/Test_orig_result_'+ str(tested_batch)
                            +'.mat', store_python_metadata=False,matlab_compatible=True)
    ...
    def test_meas(self):
        ...
        for tested_batch in range(self.test_size):
            ...
            matcontent_v[u'pred'],matcontent_v[u'meas'] = np.squeeze(test_output['pred_orig']), np.squeeze(measure_test)
            hdf5storage.write(matcontent_v,'.',self.log_dir+'/Test_meas_result_'+str(tested_batch)+'.mat',
                                          store_python_metadata=False,matlab_compatible=True)
        # matcontent_v[u'pred'],matcontent_v[u'meas'], matcontent_v[u'truth']= list_p, list_m
        # hdf5storage.write(matcontent_v,'.',self.log_dir+'/Test_result.mat',
        #                   store_python_metadata=False,matlab_compatible=True)
        print("Testing Meas Finished")
